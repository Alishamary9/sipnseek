    ...
    public class GraphLocalization : EditorWindow
    {
        ...
        void OnGUI() {
            ...
            if ( GUILayout.Button("Export TSV") ) {
                var path = EditorUtility.SaveFilePanelInProject("Export to CSV", targetGraph.agent ? targetGraph.agent.name : targetGraph.name, "tsv", string.Empty);
                var delimiter = "\t";
            ...
            if ( GUILayout.Button("Import TSV") ) {
                var path = EditorUtility.OpenFilePanel("Import TSV", "Assets", "tsv");
                if ( !string.IsNullOrEmpty(path) ) {
